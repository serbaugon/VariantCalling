...
## 3'-UTR
types_3UTR = c("3_prime_UTR_variant")
count_3UTR = subset(total_results, Type %in% types_3UTR)
sum(count_3UTR$Count)

## Exonic
# Synonymous
types_synonymous = c("synonymous_variant", "splice_region_variant&synonymous_variant")
count_synonymous = subset(total_results, Type %in% types_synonymous)
sum(count_synonymous$Count)

# Missense
types_missense = c("missense_variant", "missense_variant&splice_region_variant")
count_missense = subset(total_results, Type %in% types_missense)
sum(count_missense$Count)

# Nonsense
types_nonsense = c("stop_gained")
count_nonsense = subset(total_results, Type %in% types_nonsense)
sum(count_nonsense$Count)

# Nonstop
types_nonstop = c("stop_lost")
count_nonstop = subset(total_results, Type %in% types_nonstop)
sum(count_nonstop$Count)

# Nonstart
types_nonstart = c("start_lost")
count_nonstart = subset(total_results, Type %in% types_nonstart)
sum(count_nonstart$Count)
...
## Intergenic
types_intergenic= c("intergenic_variant", "downstream_gene_variant", "upstream_gene_variant", "TF_binding_site_variant")
count_intergenic = subset(total_results, Type %in% types_intergenic)
sum(count_intergenic$Count)

total_sum = sum(count_5UTR$Count, count_3UTR$Count, count_nonsense$Count, count_missense$Count, count_synonymous$Count, count_nonstop$Count, count_nonstart$Count, count_intronic$Count, count_intergenic$Count)
print(total_sum)
...
stacked_bar_data <- total_results %>%
  mutate(Type = ifelse(Type %in% types_5UTR, "5'-UTR",
                       ifelse(Type %in% types_3UTR, "3'-UTR",
                              ifelse(Type %in% types_synonymous, "Exonic (Synonymous)",
                                     ifelse(Type %in% types_missense, "Exonic (Missense)",
                                            ifelse(Type %in% types_nonsense, "Exonic (Nonsense)",
                                                   ifelse(Type %in% types_nonstop, "Exonic (Nonstop)",
                                                          ifelse(Type %in% types_nonstart, "Exonic (Nonstart)",
                                                                 ifelse(Type %in% types_intronic, "Splicing site", "Intergenic")))))))))

# Modify sample names in stacked_bar_data
...
# Filter to excluse "Intergenic and Synonymous"
filtered_data <- stacked_bar_data %>%
  filter(Type != "Intergenic" & Type != "Exonic (Synonymous)")

# Create the graphic
ggplot(filtered_data, aes(x = File, y = Count, fill = Type)) +
  geom_bar(stat = "identity") +
  labs(title = "Non-synonymous SNPs location",
       x = "Sample", y = "Frequency",
       fill = "Location") +
  scale_fill_manual(values = c("lightgoldenrod1","darkseagreen3", "orchid3", "blue1", "pink1", "green2", "chocolate1")) +  
  theme_minimal() +
...
# Write the results
write.table(total_results, file = "genes_P_LP_SNPs.txt", quote = FALSE, row.names = FALSE, sep = "\t")
